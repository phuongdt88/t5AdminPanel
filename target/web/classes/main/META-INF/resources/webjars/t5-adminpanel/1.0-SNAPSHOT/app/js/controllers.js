...
angular.module('xenon.controllers', []).
	controller('LoginCtrl', function($scope, $http, $location, $rootScope, $cookies)
	{
		...
		$scope.login = function() {
			...
			if(usernameVal == "" || passwordVal == "") {
				...
				var dataObj = {};
				dataObj["username"] = usernameVal;
				dataObj["password"] = passwordVal;
				$http({
					method: 'POST',
					url: 'checklogin',
					data: dataObj
	...
	controller('LogOutCtrl', function($scope, $rootScope, $cookies, $location) {
	...
	controller('TimeZoneCtrl', function($scope, $rootScope, $cookies, $http) {
		...
		$scope.updateTimeZone = function() {
			...
			var dataObj = {};
			dataObj["newTimeZone"] = newTimeZone;
			dataObj["username"] = $cookies.username;
			$http({
				method: 'POST',
				url: 'updatetimezone',
				data: dataObj
	...
	controller('usersmanagementCtrl', function($scope, $rootScope, $modal, $sce) {
		// Open Simple Modal
    ...
    $body.attr('class', $body.attr('class').replace(/\sskin-[a-z]+/)).addClass('skin-' + $layout.get('skins.sidebarMenu'));
//    		$layout.loadOptionsFromCookies();
		// remove this line if you don't want to support cookies that remember layout changes
	...
	controller('SidebarMenuCtrl', function($scope, $rootScope, $menuItems, $timeout, $location, $state, $layout)
	{
		...
		$sidebarMenuItems.setActive($scope.menuItems, $location.path() );

		$rootScope.$on('$stateChangeSuccess', function()
		{
			$sidebarMenuItems.setActive($scope.menuItems, $state.current.name);
		});
	...
	controller('ThemeSkinsCtrl', function($scope, $layout)
	{
		...
		$scope.skins = [
			...
			{value: 'turquoise', 	name: 'Truquoise'		,	palette: ['#3E253E','#96ead9','#FFFFFF','#1daf92','#3E253E','#0f7e68']},
